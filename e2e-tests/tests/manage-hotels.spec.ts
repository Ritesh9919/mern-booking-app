...
test("should display hotels", async ({ page }) => {
  ...
  await expect(page.getByText("Hotel1")).toBeVisible();
...
test("should edit hotel", async ({ page }) => {
  ...
  await expect(page.locator("[name=name]")).toHaveValue("Test1");
  await page.locator("[name=name]").fill("Test1 UPDATED");
  await page.getByRole("button", { name: "Save" }).click();
  await expect(page.getByText("Hotel updated")).toBeVisible();
  await page.reload();

  await expect(page.locator("[name=name]")).toHaveValue("Test1 UPDATED");
